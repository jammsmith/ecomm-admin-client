...
export const validateInventoryFields = (requestedFields, type) => {
  ...
  switch (type) {
    ...
    default:
      throw new Error(
        `Invalid type '${type}' supplied to validateInventoryFields`
      )
  }
  ...
  for (const field of requiredFields) {
    if (field === 'numInStock') {
      if (typeof requestedFields[field] !== 'number') {
        failedItems.push(field)
      }
    } else if (!requestedFields[field]) {
      failedItems.push(field)
    }
  }
...
export const getInitialFormFields = (type) => {
  switch (type) {
    ...
    default:
      throw new Error(
        `Invalid inventory type '${type}' supplied to getInitialFormFields`
      )
  }
}
...
export const getFormattedRow = (item, type) => {
  ...
  switch (type) {
    ...
    default:
      throw new Error(
        `Invalid inventory type '${type}' supplied to getFormattedRow`
      )
  }
}
